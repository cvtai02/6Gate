...
const PROVIDER_META: Record<string, ProviderMeta> = {
  ...
  zernio: {
    ...
    showScopes: false,
    notes: [
      "This provider only stores shared Zernio settings.",
      "Leave API Base URL blank to use https://zernio.com/api/v1.",
      "Add one or more Zernio accounts below. Each account has its own API key.",
    ],
    Icon: ZernioIcon,
  },
};
...
function ConfigureAppModal({
  ...
  const clientIdLabel = meta.credentialLabels?.clientId ?? "Client ID";
  const clientSecretLabel = meta.credentialLabels?.clientSecret ?? "Client Secret";
  const clientIdRequired = meta.clientIdRequired ?? true;
  const clientSecretRequired = meta.clientSecretRequired ?? true;
  const showClientSecret = meta.showClientSecret ?? true;
  const showOAuthRedirect = meta.showOAuthRedirect ?? true;
  const showScopes = meta.showScopes ?? true;
    ...
    >
      <div className="bg-[var(--muted)] border border-[var(--border)] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className={`grid gap-4 ${showClientSecret ? "grid-cols-2" : "grid-cols-1"}`}>
            <div>
              <label className="block text-xs text-gray-400 mb-1.5">{clientIdLabel}</label>
              <input
                required={clientIdRequired}
                value={form.clientId}
                onChange={(e) => setForm((f) => ({ ...f, clientId: e.target.value }))}
                className="w-full bg-black/30 border border-[var(--border)] focus:border-indigo-500 rounded-lg px-3 py-2 text-sm text-white focus:outline-none transition-colors"
                placeholder={clientIdRequired ? "required" : "optional"}
              />
            </div>
            {showClientSecret && (
              <div>
                <label className="block text-xs text-gray-400 mb-1.5">{clientSecretLabel}</label>
                <input
                  type="password"
                  required={!isEdit && clientSecretRequired}
                  value={form.clientSecret}
                  onChange={(e) => setForm((f) => ({ ...f, clientSecret: e.target.value }))}
                  className="w-full bg-black/30 border border-[var(--border)] focus:border-indigo-500 rounded-lg px-3 py-2 text-sm text-white focus:outline-none transition-colors"
                  placeholder={isEdit ? "leave blank to keep current" : clientSecretRequired ? "required" : "optional"}
                />
              </div>
            )}
          </div>
  ...
  return (
    ...
    >
      <div className="bg-[var(--muted)] border border-[var(--border)] rounded-2xl w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <p className="text-xs text-gray-500">
            Each local Zernio account uses its own API key. After saving, connected Zernio social accounts will be imported as destinations.
          </p>
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1.5">Zernio API Key</label>
            <input
              type="password"
              required
              value={form.apiKey}
...
function ProviderSection({
  ...
  const [showZernioConnect, setShowZernioConnect] = useState(false);
  const [syncing, setSyncing] = useState(false);
  ...
  async function handleSync() {
    ...
    try {
      if (isZernioProvider) {
        const res = await fetch("/api/accounts/zernio/sync", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (!res.ok) {
          ...
          setSyncMsg({
            kind: "ok",
            text: `${data.accounts ?? 0} account${data.accounts === 1 ? "" : "s"}, ${data.destinations ?? 0} destination${data.destinations === 1 ? "" : "s"}`,
          });
          onDestinationsChanged();
        }
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--muted)] overflow-hidden">
      ...
      {showZernioConnect && (
        <ZernioAccountModal
          provider={provider}
          onClose={() => setShowZernioConnect(false)}
          onConnected={() => {
            setShowZernioConnect(false);
            onDestinationsChanged();
          }}
        />
      )}
        ...
        <div className="flex items-center gap-2">
          ...
          {accounts.length > 0 && (
            <button
              ...
              className="text-xs text-gray-400 hover:text-white px-3 py-1.5 rounded-lg border border-[var(--border)] hover:border-gray-500 transition-colors disabled:opacity-50"
            >
              {syncing ? "Syncing..." : isZernioProvider ? "Sync Accounts" : "Sync"}
            </button>
          )}
          {isFacebookProvider && (
            <button
          ...
          {isZernioProvider ? (
            <button
              onClick={() => setShowZernioConnect(true)}
        ...
        {accounts.length === 0 ? (
          <div className="py-6 text-center">
            ...
            <p className="text-xs text-gray-600 mt-0.5">
              {isZernioProvider ? "Add a Zernio account with its own API key." : "Add your first connection to get started"}
            </p>
            <button
              onClick={() => isZernioProvider ? setShowZernioConnect(true) : onConnect(provider.id)}
              disabled={syncing}
              className="mt-3 text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-1.5 rounded-lg transition-colors disabled:opacity-50"
            >
              {isZernioProvider ? "+ Add Account" : "+ Add Connection"}
            </button>
          </div>
        ) : (
          accounts.map((acc) => (
    ...
    <div className="p-8 flex flex-col gap-6 min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className={`w-14 h-14 rounded-2xl ${iconBg} flex items-center justify-center shrink-0 overflow-hidden`}>
            <Icon className={type === ProviderType.zernio ? "w-14 h-14" : "w-7 h-7 text-white"} />
          </div>
          <div>
      ...
      {providers.length === 0 ? (
        <div className="rounded-xl border border-[var(--border)] bg-[var(--muted)] p-12 text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {type === ProviderType.zernio
              ? "Configure shared Zernio settings, then add accounts with API keys."
